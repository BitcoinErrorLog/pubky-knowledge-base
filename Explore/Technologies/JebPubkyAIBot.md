...
Jeb operates as an autonomous agent on the Pubky network, monitoring mentions and providing helpful AI-generated responses. He reads only public Pubky data. The public text of the posts he is asked about — and his tool results from Nexus Scout and, when used, web search — is sent to a third-party model API, currently Moonshot AI (Kimi K3), to generate the answer. Synonym does not store user data beyond the public evidence bundle Jeb publishes and operational logs. No private messages or private data are read. Replies live as ordinary public posts on [[Homeservers|homeservers]] under Jeb's key.

### Key Characteristics

- **AI-Powered**: Answers generated by Moonshot AI (Kimi K3); Synonym controls which model is available
...
- **Privacy-Preserving**: Reads only public Pubky data; public thread text and tool results go to Moonshot to generate answers; no private messages or private data are read
...
3. Runs `search_web` (Moonshot `$web_search` by default, or Brave Search API) and cites the result URLs
...
- **AI Provider**: Moonshot AI (Kimi K3); Synonym-controlled, no user-selectable provider list
- **Web Search**: Moonshot `$web_search` builtin (default); Brave Search API as the alternative backend
...
```bash
# Web search (default: Moonshot $web_search on the same JEB_MODEL_* credentials)
JEB_WEB_PROVIDER=moonshot
JEB_BRAVE_API_KEY=
JEB_SWITCH_WEB=0
...
Production Jeb uses **Moonshot AI (Kimi K3)** through an OpenAI-compatible chat endpoint. The reason process reads these names from `src/config.ts` / `.env.example`:
...
If `JEB_MODEL` is unset, the code default is `gpt-4o-mini`; ops pin `kimi-k3`. Moonshot kimi-k3 requires sampling temperature `1` (`JEB_MODEL_TEMPERATURE` unset means 1). Synonym controls which model is available; there is no user-selectable Groq, OpenAI, Anthropic, or OpenRouter list.

Self-hosting operators may point `JEB_MODEL_BASE_URL` at another OpenAI-compatible endpoint and set `JEB_MODEL` / `JEB_MODEL_API_KEY` for that runtime. That is an operator exit, not a public provider picker.
...
Jeb's `search_web` tool (`packages/bot-kit/src/web/`) looks up current events for `research_web` and evidence maps. It returns titles, URLs, and snippets and does not fetch arbitrary pages. Those URLs are cited in the reply.

**Default — Moonshot `$web_search`:** `JEB_WEB_PROVIDER=moonshot` (the default) calls Moonshot's builtin `$web_search` on the same `JEB_MODEL_*` credentials.

**Alternative — Brave Search API:** `JEB_WEB_PROVIDER=brave` with `JEB_BRAVE_API_KEY`. Direct HTTPS to `api.search.brave.com`. There is no Brave MCP server.

**Off / kill switch:** `JEB_WEB_PROVIDER=off` unregisters the tool. The `web` kill switch (`JEB_SWITCH_WEB`, and the global switch) returns unavailable. Failures never fabricate sources. Caps: `JEB_WEB_PER_MENTION_CAP` (default 2) and `JEB_WEB_DAILY_CEILING` (default 200).
...
- **Web Search**: Moonshot `$web_search` by default; Brave Search API is the alternative. Gated by `JEB_SWITCH_WEB`
...
- **Public Interactions**: All mentions and replies are public. Jeb reads only public Pubky data; no private messages or private data are read
- **AI Provider**: The public text of the posts Jeb is asked about, plus Nexus Scout tool results and (when used) web-search results, is sent to a third-party model API — currently Moonshot AI (Kimi K3) — to generate the answer. No other model provider receives that text
- **Search Queries**: When web search is used, result snippets are included in the model prompt; fetched pages are not followed beyond the search result
- **Stored Data**: Synonym does not store user data beyond the public evidence bundle Jeb publishes and operational logs
...
- **Moonshot AI (Kimi K3)**: `JEB_MODEL_BASE_URL=https://api.moonshot.ai/v1`

### Related Tools

- **BIP39 Mnemonic Generator**: [https://iancoleman.io/bip39/](https://iancoleman.io/bip39/)
- **Brave Search API**: [https://brave.com/search/api/](https://brave.com/search/api/) (alternative `JEB_WEB_PROVIDER=brave` backend)
